...
result = ''
try {
let data = await axios.get("https://clp.pw/api/v1/create?url="+encodeURIComponent(url));
rssult = data.result.url
} catch {
result = url
}
}
  ...
  return new Promise((resolve, reject) => {
    ytdl
      ...
      .then(async (getUrl) => {
        ...
        let tinyUrl = await link(resultFix[0].audio);
        let title = getUrl.videoDetails.title;
        let desc = getUrl.videoDetails.description;
        let views = parseInt(getUrl.videoDetails.viewCount || 0);
        let likes = getUrl.videoDetails.likes;
