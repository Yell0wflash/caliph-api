...
async function start() {
  if (opts.path) {
    ...
    let allow_ext = ["js",
      "html",
      "css"];
    let filters = getFiles(dir).filter(a => allow_ext.includes(a.split(".").pop()) && !/node_modules/gi.test(a));
    for (let i of filters) {
      let latency = new Date();
      try {
        let minifyres = await (await import("minify")).minify(i);
        await fs.promises.writeFile(i, minifyres);
        console.log(`File ${i.split("/").pop()} Success Compressed, Latency : ${new Date - latency}ms`)
      } catch (e) {
